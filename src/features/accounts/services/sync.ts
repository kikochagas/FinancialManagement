...
export async function internalSyncBalance(accountId: string, userId: string) {
  ...
  try {
    ...
    if (
      e.message === "No supported balance type available" ||
      e.message === "Currency mismatch between provider and account"
    ) {
      throw e;
    }
...
export async function internalSyncTransactions(
  accountId: string,
  userId: string,
) {
  const account = await db.account.findUnique({
  ...
  if (!mapping.transactionImportFrom && !mapping.lastTransactionSyncedAt) {
    ...
    mapping.transactionImportFrom &&
    !mapping.lastTransactionSyncedAt
  ) {
    strategy = "default";
    ...
    const overlapDate = new Date(
      mapping.lastTransactionSyncedAt.getTime() - 7 * 24 * 60 * 60 * 1000,
    );
