...
describe("broker-import database integration", () => {
  ...
  beforeAll(async () => {
    user = await db.user.create({
      data: {
        email: "test-broker-import@example.com",
        passwordHash: "x",
        name: "Test User",
      },
    });
  ...
  test("valid event persists securely", async () => {
    ...
    const saved = await db.investmentEvent.findFirst({
  ...
  test("invalid date is rejected server-side and rolls back", async () => {
    ...
    await expect(
      importBrokerTransactionsForUser(user.id, account.id, tx),
    ).rejects.toThrow(/Strict server validation failed/);
  });
  ...
  test("account ownership is enforced", async () => {
    ...
    await expect(
      importBrokerTransactionsForUser(otherUser.id, account.id, tx),
  ...
  test("active open banking connection blocks import", async () => {
    ...
    await db.externalAccountMapping.create({
      data: {
        ...
        providerAccountUid: "123",
        identificationHash: "123",
      },
    });
    ...
    await expect(
      importBrokerTransactionsForUser(user.id, obAccount.id, tx),
    ).rejects.toThrow(
      /Cannot import broker transactions into an actively connected Open Banking account/,
    );
  ...
  test("idempotent reimport + balance", async () => {
    ...
    // First import
    const res1 = await importBrokerTransactionsForUser(
      user.id,
      account.id,
      tx,
      true,
    ...
    const res2 = await importBrokerTransactionsForUser(
      user.id,
      account.id,
      tx,
      true,
  ...
  test("unsafe balance is rejected", async () => {
    ...
    await expect(
      importBrokerTransactionsForUser(user.id, account.id, tx, true),
    ).rejects.toThrow(/Cash balance calculation was unsafe/);
