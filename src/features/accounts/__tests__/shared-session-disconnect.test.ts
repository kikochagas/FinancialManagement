...
vi.mock("@/lib/banking/enable-banking-client", () => {
  ...
  EnableBankingClientMock.prototype.revokeSession = vi
    .fn()
    .mockResolvedValue(undefined);
  EnableBankingClientMock.prototype.getBalances = vi.fn().mockResolvedValue([]);
  EnableBankingClientMock.prototype.normalizeBalance = vi
    .fn()
    .mockReturnValue(null);
  EnableBankingClientMock.prototype.getTransactions = vi
    .fn()
...
describe("Shared Session Disconnect", () => {
  ...
  test("Disconnecting Francisco does not revoke the session", async () => {
    const { EnableBankingClient } =
      await import("@/lib/banking/enable-banking-client");
    ...
    const mappingA = await db.externalAccountMapping.findUnique({
      where: { id: mappingAId },
    });
    expect(mappingA?.disconnectedAt).not.toBeNull();

    // Verify Mapping B remains connected
    const mappingB = await db.externalAccountMapping.findUnique({
      where: { id: mappingBId },
    });
    ...
    const accA = await db.account.findUnique({
      where: { id: accountFranciscoId },
    });
    ...
    expect((syncResA?.data as any)?.error).toBe(
      "Account is not linked to any external provider",
    );

    const syncResB = await syncBankAccount({ accountId: accountMillenniumId });
    expect((syncResB?.data as any)?.error).not.toBe(
      "Account is not linked to any external provider",
    );
  });
  ...
  test("Disconnecting Millennium (the last one) revokes the session", async () => {
    ...
    expect(EnableBankingClient.prototype.revokeSession).toHaveBeenCalledWith(
      "session-1",
    );
  ...
  test("Disconnecting Millennium fails if network error, leaves connection intact", async () => {
    ...
    // Mock network error
    (EnableBankingClient.prototype.revokeSession as any).mockRejectedValueOnce(
