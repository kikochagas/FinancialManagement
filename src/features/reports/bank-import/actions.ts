...
const importBankStatementSchema = z
  .object({
    ...
    transactions: z
      .array(
        z.object({
          bookingDate: z
            .string()
            .regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid calendar date"),
  ...
  .refine(
    function (data) {
      if (
        data.updateBalance &&
        (data.endingBalance === null || data.endingBalance === undefined)
      ) {
        return false;
      }
      return true;
...
export const importBankStatementAction = authActionClient
  ...
  .action(async ({ parsedInput, ctx: { userId } }) => {
    const { accountId, updateBalance, endingBalance, transactions } =
      parsedInput;
    ...
    if (hasActiveMapping) {
      throw new Error(
        "Cannot import unstructured bank statement into an actively connected Open Banking account. Please disconnect it first.",
      );
    }
...
export const previewBankStatementDuplicatesAction = authActionClient
  ...
  .action(async ({ parsedInput, ctx: { userId } }) => {
    ...
    const dates = [
      ...new Set(transactions.map((t) => new Date(t.bookingDate).getTime())),
    ].map((t) => new Date(t));
