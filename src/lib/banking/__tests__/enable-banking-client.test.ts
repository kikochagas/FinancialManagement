...
describe("EnableBankingClient Contract", () => {
  ...
  it("JWT generation follows contract (kid, issuer, audience)", async () => {
    ...
    const res = await client.createAuthorization(
      "Banco BPI",
      "PT",
      "http://localhost/cb",
      "state123",
  ...
  it("createAuthorization rejects invalid maximumConsentValiditySeconds", async () => {
    ...
    await expect(
      client.createAuthorization("Bank", "PT", "cb", "state", 0),
    ).rejects.toThrow(
      "maximumConsentValiditySeconds must be a valid positive finite number.",
    );
    await expect(
      client.createAuthorization("Bank", "PT", "cb", "state", -10),
    ).rejects.toThrow(
      "maximumConsentValiditySeconds must be a valid positive finite number.",
    );
    await expect(
      client.createAuthorization("Bank", "PT", "cb", "state", NaN),
    ).rejects.toThrow(
      "maximumConsentValiditySeconds must be a valid positive finite number.",
    );
    await expect(
      client.createAuthorization("Bank", "PT", "cb", "state", Infinity),
    ).rejects.toThrow(
      "maximumConsentValiditySeconds must be a valid positive finite number.",
    );
  });
  ...
  it("Throws EnableBankingProviderError on 401/403", async () => {
    ...
    await expect(client.getBalances("acc-1")).rejects.toThrow(
      EnableBankingProviderError,
    );
    await expect(client.getBalances("acc-1")).rejects.toThrow(
      'Enable Banking API error: 401 {"error":"session_expired"}',
    );
  });
