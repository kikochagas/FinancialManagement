export function buildTransactionPayload(formState: any) {
  ...
  return {
    ...
    destinationAccountId:
      isTransfer && formState.destinationAccountId
...
export function getTransactionPerspective(
  tx: { direction: string; accountId: string; destinationAccountId: string },
  filterId: string,
) {
  if (tx.direction === "InternalTransfer") {
    if (filterId === tx.accountId)
      return { type: "outgoing", sign: "-", color: "text-foreground" };
    if (filterId === tx.destinationAccountId)
      return {
        type: "incoming",
        sign: "+",
...
export function filterTransactions(
  ...
  return transactions.filter((tx) => {
    ...
    const matchesDirection =
      ...
      tx.direction === filters.directionFilter;
    const matchesAccount =
      filters.accountFilter === "all" ||
      tx.accountId === filters.accountFilter ||
      (tx.direction === "InternalTransfer" &&
        tx.destinationAccountId === filters.accountFilter);
    const matchesCategory =
      filters.categoryFilter === "all" ||
      tx.categoryId === filters.categoryFilter;
    return (
      matchesSearch && matchesDirection && matchesAccount && matchesCategory
    );
  });
}
