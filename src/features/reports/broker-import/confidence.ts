...
export function evaluateBrokerMappingConfidence(
  ...
  if (
    !mappedSemantics.includes("DATETIME") &&
    !mappedSemantics.includes("DATE")
  ) {
    score -= 0.4;
  }
  ...
  if (
    !mappedSemantics.includes("AMOUNT") &&
    !mappedSemantics.includes("QUANTITY")
  ) {
    score -= 0.3;
  }

  // Detect duplicates
  const nonNull = mappedSemantics.filter(
    (s) => s && s !== "IGNORE" && s !== "UNMAPPED",
  );
