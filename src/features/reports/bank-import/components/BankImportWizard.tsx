...
import {
  ...
  mapBankStatementColumnsWithAIAction,
  previewBankStatementDuplicatesAction,
} from "../actions";
...
export function BankImportWizard({
  ...
  const [transactions, setTransactions] = useState<
    (ParsedBankTransaction & { import: boolean; categoryId?: string })[]
  >([]);
  ...
  const [statementCurrency, setStatementCurrency] = useState<string | null>(
    null,
  );
  const [statementCurrencyStatus, setStatementCurrencyStatus] =
  ...
  const proceedToMapping = async () => {
    ...
    const aiMapper = new BankStatementAIMapper(
      // Mock Provider that calls our Server Action
      {
        generateStructured: async (req: any) => {
          ...
          const res = await mapBankStatementColumnsWithAIAction({
            columns: cols,
          });
          if (res?.data?.success) {
    ...
    const {
      ...
      aiError,
      warnings,
    } = await orchestrateColumnMapping(
      headers,
      fileData,
      aiMapper,
      headerRowIdx,
  ...
  const proceedToAccount = () => {
    ...
    const {
      transactions: txs,
      endingBalance: detectedBalance,
      footerRowsSkipped,
      blankRowsIgnored,
      statementCurrencyStatus: currStatus,
      statementCurrency: curr,
    } = buildTransactions(fileData, headerRowIdx, mapping);
  ...
  const proceedToReview = async () => {
    ...
    const validCandidates = transactions
      .map((t, idx) => ({ t, idx }))
      .filter(({ t }) => t.valid);
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        setTransactions(
          (prev) =>
            applyCategorySuggestionsAndFallbacks(
              prev,
              duplicates,
              categoriesMap,
              categories,
  ...
  const revalidateRow = (
    idx: number,
    updates: Partial<(typeof transactions)[0]>,
  ) => {
    ...
    let isProbableDuplicate =
      tx.isProbableDuplicate === true ? true : undefined;
    if (
  ...
  const handleImport = () => {
    ...
    startTransition(async () => {
      ...
      if (res?.data?.success) {
        ...
        const unresolvedInvalidRows = transactions.filter(
          (t) => !t.valid,
        ).length;
        const probableDuplicatesExcludedByPreview = transactions.filter(
          (t) => t.valid && t.isProbableDuplicate && !t.import,
        ).length;
        const excludedByUser = transactions.filter(
          (t) => t.valid && !t.isProbableDuplicate && !t.import,
        ).length;
        ...
        setSummary((prev: any) => ({
          ...
          probableDuplicatesSkippedByServer:
            res.data!.probableDuplicatesSkipped,
      ...
      <CardContent className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            {headerRowIdx !== null ? (
              <p className="text-sm text-muted-foreground">
                Detected headers on row {headerRowIdx + 1}
              </p>
            ) : (
              <div>
                ...
                <label className="text-sm">
                  Transaction headers start on row (1-indexed):
                </label>
                <Input
                  type="number"
                  min={1}
                  ...
                  onChange={(e) => {
                    ...
                    if (
                      ...
                      const validCellCount = rowData.filter(
                ...
                {headerInputRaw !== "" && headerRowIdx === null && (
                  <div className="text-red-500 text-xs mt-1">
                    Invalid row number or insufficient header columns (requires
            ...
            <Button
              ...
              disabled={!selectedSheet || headerRowIdx === null}
            >
              Continue
            </Button>
          </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            {aiAttempted && aiWarnings.length > 0 && (
              <div className="p-3 bg-yellow-50 text-yellow-800 border border-yellow-200 rounded-md text-sm">
                <p className="font-medium mb-1">
                  AI-assisted mapping is currently unavailable.
                </p>
                <p>
                  You can continue by reviewing the column mapping manually.
                </p>
              </div>
            )}
                ...
                >
                  <div className="flex-1">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">
                      ({m.source} {(m.confidence * 100).toFixed(0)}%)
                    </span>
                  </div>
                  <select
            ...
            {(() => {
              const sems = Object.values(mapping)
                .map((m) => m.semantic)
                .filter(Boolean);
              const counts: Record<string, number> = {};
              sems.forEach(
                (s) => (counts[s as string] = (counts[s as string] || 0) + 1),
              );
              const hasCollision = Object.entries(counts).some(
                ([sem, count]) =>
                  count > 1 && !["IGNORE", "UNMAPPED"].includes(sem),
              );
              const missingReq =
                !counts["BOOKING_DATE"] ||
                !counts["DESCRIPTION"] ||
                (!counts["AMOUNT"] && !(counts["DEBIT"] && counts["CREDIT"]));

              if (hasCollision)
                return (
                  <div className="text-red-500 text-sm">
                    Please resolve mapping collisions (duplicate semantics)
                    before continuing.
                  </div>
                );
              if (missingReq)
                return (
                  <div className="text-red-500 text-sm">
                    Missing required mappings: Booking Date, Description, and
                    Amount (or Debit/Credit).
                  </div>
                );
              return (
                <Button onClick={proceedToAccount}>Confirm Mappings</Button>
              );
            })()}
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-4">
              ...
              {localAccounts.map((a) => {
                ...
                return (
                  <option key={a.id} value={a.id} disabled={isActiveLink}>
                    {a.name} ({a.balance}){" "}
                    {isActiveLink
                      ? "(Currently synchronized via Open Banking)"
                      : ""}
                  </option>
                );
              })}
              ...
              <div className="p-4 border rounded bg-muted/20 space-y-3 mt-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      className="w-full p-2 border rounded-md h-9 text-sm"
                      value={newAccType}
            ...
            {endingBalance !== null && (
              <div className="mt-4 p-4 border rounded bg-muted/30 space-y-2">
                <p className="text-sm font-medium">
                  Statement Ending Balance detected: {endingBalance.toFixed(2)}{" "}
                  {statementCurrency ||
                    (selectedAccountId &&
                ...
                <label className="flex items-center space-x-2 text-sm">
                  ...
                  />
                  <span>
                    Update account balance to exactly match this statement
                    ending balance.
                  </span>
                </label>
              </div>
            ...
            {endingBalance === null && (
              <div className="mt-4 p-4 border rounded bg-red-50/50 space-y-2">
                <p className="text-sm font-medium text-red-600">
                  Statement ending balance could not be determined confidently.
                </p>
              </div>
            )}
            ...
            {statementCurrencyStatus === "ambiguous" && (
              <div className="mt-4 p-4 border rounded bg-red-50/50 space-y-2">
                <p className="text-sm font-medium text-red-600">
                  This statement contains multiple currencies. Multi-currency
                  statement import is not supported yet.
                </p>
              </div>
            )}
            ...
            {statementCurrencyStatus === "detected" &&
              ...
              selectedAccountId &&
              (() => {
                const selAcc = localAccounts.find(
                  (a) => a.id === selectedAccountId,
                );
                if (
                  selAcc &&
                  selAcc.currency.toUpperCase() !==
                    statementCurrency.toUpperCase()
                ) {
                  return (
                    <div className="mt-4 p-4 border rounded bg-red-50/50 space-y-2">
                      <p className="text-sm font-medium text-red-600">
                        Statement currency ({statementCurrency}) does not match
                        the selected account ({selAcc.currency}).
                      </p>
                    </div>
                  );
                }
          ...
          <div className="space-y-4">
            ...
            {categories.length === 0 && (
              <div className="p-4 border border-red-200 bg-red-50 text-red-600 rounded-md text-sm font-medium">
                Categories could not be loaded. Please reload the page and try
                again.
              </div>
            )}
            <p className="text-sm text-muted-foreground">
              {transactions.filter((t) => t.import && t.valid).length} ready to
              import.
            </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {transactions.map((tx, idx) => (
                    <tr
                      key={idx}
                      className={`border-b ${!tx.valid ? "bg-red-50/50" : ""} ${tx.isProbableDuplicate ? "bg-orange-50/30" : ""}`}
                    >
                      <td className="p-2">
                        <input
                          type="checkbox"
                          ...
                          disabled={!tx.valid}
                          onChange={(e) =>
                            revalidateRow(idx, { import: e.target.checked })
                          }
                        />
                        {!!tx.isProbableDuplicate && (
                          <span className="text-[10px] bg-orange-200 text-orange-800 px-1 ml-2 rounded">
                            Probable duplicate
                          </span>
                        )}
                      </td>
                      <td className="p-2">
                        <Input
                          ...
                          onChange={(e) =>
                            revalidateRow(idx, { bookingDate: e.target.value })
                          }
                        />
                      </td>
                      <td className="p-2">
                        <Input
                          ...
                          onChange={(e) =>
                            revalidateRow(idx, { description: e.target.value })
                          }
                        />
            ...
            <Button
              ...
              disabled={
                isPending ||
                transactions.filter((t) => t.import && t.valid).length === 0 ||
                categories.length === 0
              }
            >
              Confirm & Import
        ...
        {step === 6 && (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm">
              Please wait while your transactions are being imported.
            </p>
          </div>
        )}
        ...
        {step === 7 && summary && (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Candidate Transactions: {summary.candidateTransactions}
            </p>
            <p className="text-sm text-muted-foreground">
              Blank Rows Ignored: {summary.blankRowsIgnored}
            </p>
            <p className="text-sm text-muted-foreground">
              Footer Rows Skipped: {summary.footerRowsSkipped}
            </p>
            <p className="text-sm text-amber-600">
              Unresolved Invalid Rows: {summary.unresolvedInvalidRows}
            </p>
            <p className="text-sm text-amber-600">
              Probable Duplicates Excluded By Preview:{" "}
              {summary.probableDuplicatesExcludedByPreview}
            </p>
            <p className="text-sm text-amber-600">
              Excluded By User: {summary.excludedByUser}
            </p>
            <p className="text-sm font-medium">
              Requested For Import: {summary.requestedForImport}
            </p>
            <p className="text-sm text-amber-600">
              Probable Duplicates Skipped By Server:{" "}
              {summary.probableDuplicatesSkippedByServer}
            </p>
            <p className="text-sm font-semibold text-emerald-600">
              Imported: {summary.imported}
            </p>
            <p className="text-sm">
              Balance Updated: {summary.balanceUpdated ? "Yes" : "No"}
            </p>
