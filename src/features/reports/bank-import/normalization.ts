...
export function resolveAmount(
  ...
  const hasDebit =
    debitRaw !== undefined &&
    debitRaw !== null &&
    String(debitRaw).trim() !== "";
  const hasCredit =
    creditRaw !== undefined &&
    creditRaw !== null &&
    String(creditRaw).trim() !== "";

  if (hasDebit && hasCredit) {
    warnings.push(
      "Both Debit and Credit columns contain values. Type is ambiguous.",
    );
    return {
      amount: null,
      direction: null,
  ...
  if (amountRaw !== undefined && amountRaw !== null && amountRaw !== "") {
    ...
    let direction = deriveTransactionDirection(
      res.value !== null ? Math.abs(res.value) : null,
      null,
      null,
      null,
    );
