...
describe("Banking Reconnect Tests", () => {
  ...
  describe("Callback Reconciliation", () => {
    it("TEST A & B & C - Reauth active account with disconnected sibling", async () => {
      ...
      mockDb.externalAccountMapping.findUnique.mockImplementation(
        ({ where }: any) => {
          console.log("findUnique called with where:", JSON.stringify(where));
          if (
            where.bankConnectionId_identificationHash?.identificationHash ===
            "hash-a"
          ) {
            return {
              id: "map-a",
      ...
      const req = new Request(
        "http://localhost/api/banking/callback?code=abc&state=good",
      );
      const res = await CallbackGET(req);

      expect(res.status).toBe(307);
      expect(res.headers.get("location")).toContain(
        "/accounts/link?connectionId=conn-1",
      );
      ...
      const callA = mockDb.externalAccountMapping.update.mock.calls.find(
        (call: any) => call[0].where.id === "map-a",
      );
    ...
    it("TEST C - Ordinary connect flow (no reconnectAccountId)", async () => {
      ...
      const call = mockDb.externalAccountMapping.update.mock.calls.find(
        (c: any) => c[0].where.id === "map-a",
      );
