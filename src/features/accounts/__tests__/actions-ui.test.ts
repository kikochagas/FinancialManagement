...
vi.mock("@/lib/banking/enable-banking-client", () => {
  ...
  EnableBankingClientMock.prototype.getBalances = vi
    .fn()
    .mockResolvedValue([
...
describe("Phase 10 UI Actions", () => {
  ...
  test("disconnectBank revokes session and updates status while preserving history", async () => {
    const accMapInitial = await db.externalAccountMapping.findFirst({
      where: { accountId },
    });
    ...
    const accMap = await db.externalAccountMapping.findFirst({
      where: { accountId },
    });
    ...
    const txMapAfter = await db.externalTransactionMapping.findUnique({
  ...
  test("syncBankAccount failure does not delete account", async () => {
    ...
    (EnableBankingClient as any).prototype.getBalances.mockRejectedValueOnce(
    ...
    expect((result?.data as any)?.error).toContain(
