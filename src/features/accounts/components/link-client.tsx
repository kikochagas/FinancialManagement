...
export function LinkAccountsClient({
  connectionId,
  institutionName,
  pendingAccounts,
  existingAccounts,
}: LinkAccountsClientProps) {
  ...
  const [selections, setSelections] = useState<
    Record<string, AccountSelectionState>
  >(() => {
  ...
  const handleSelectionChange = (
    pendingId: string,
    field: keyof AccountSelectionState,
    value: any,
  ) => {
    setSelections((prev) => ({
      ...prev,
      ...
      try {
        ...
        if (res?.data?.success) {
          ...
          if (linkedIds.length > 0) {
            ...
            if (anyReauth) {
              alert(
                "Bank account connected, but authorization must be renewed before it can be synchronized.",
              );
            } else if (anyError && !anySuccess && !anyPartial) {
              alert(
                "Bank account connected, but the initial synchronization failed. You can retry from Accounts.",
              );
            } else if (anyPartial || (anyError && anySuccess)) {
              alert(
                "Bank account connected, but some data could not be synchronized.",
              );
            } else {
  ...
  const accAlreadyLinked = (id: string) =>
    pendingAccounts.find((a) => a.id === id)?.isAlreadyLinked;

  const allAccountsLinked =
    pendingAccounts.length > 0 &&
    pendingAccounts.every((a) => a.isAlreadyLinked);
    ...
    return (
      <Card className="border-border max-w-lg mx-auto mt-12 text-center">
        <CardHeader>
          ...
          <CardDescription>
            Your connection to {institutionName} was successfully restored. All
      ...
      <div className="space-y-4">
        {pendingAccounts.map((acc) => {
          ...
          return (
            <Card
              key={acc.id}
              className={`border-border transition-colors ${isSelected ? "bg-card" : "bg-muted/30 opacity-75"}`}
            >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-xs mt-0.5">
                      {acc.maskedIdentifier || "No account number"} •{" "}
                      {acc.currency}
                    </CardDescription>
                  </div>
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked: boolean) =>
                          handleSelectionChange(
                            acc.id,
                            "action",
                            checked ? LinkAction.CREATE : LinkAction.IGNORE,
                          )
                        }
                      />
                      <Label
                        htmlFor={`connect-${acc.id}`}
              ...
              {isSelected && !acc.isAlreadyLinked && (
                <CardContent className="pt-4 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Create New Option */}
                    <div
                      className={`p-4 rounded-lg border ${state.action === LinkAction.CREATE ? "border-primary bg-primary/5" : "border-border bg-background"} flex gap-3 cursor-pointer`}
                      onClick={() =>
                        handleSelectionChange(
                          acc.id,
                          "action",
                    ...
                    >
                      <div className="pt-0.5">
                        <div
                          className={`h-4 w-4 rounded-full border flex items-center justify-center ${state.action === LinkAction.CREATE ? "border-primary" : "border-input"}`}
                        >
                    ...
                    <div
                      className={`p-4 rounded-lg border ${state.action === LinkAction.LINK ? "border-primary bg-primary/5" : "border-border bg-background"} flex gap-3 cursor-pointer`}
                      onClick={() =>
                        handleSelectionChange(acc.id, "action", LinkAction.LINK)
                      }
                    >
                      <div className="pt-0.5">
                        <div
                          className={`h-4 w-4 rounded-full border flex items-center justify-center ${state.action === LinkAction.LINK ? "border-primary" : "border-input"}`}
                        >
                          {state.action === LinkAction.LINK && (
                            <div className="h-2 w-2 rounded-full bg-primary" />
                          )}
                        </div>
                      </div>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            {existingAccounts.filter(
                              (ea) => ea.currency === acc.currency,
                            ).length === 0 && (
                              <SelectItem value="none" disabled>
                                No compatible EUR accounts
                              </SelectItem>
                            )}
                          </SelectContent>
                        </Select>
                    ...
                    <div className="flex flex-row items-start space-x-2 bg-muted/20 p-3 rounded-md">
                      <Checkbox
                        ...
                        onCheckedChange={(c: boolean) =>
                          handleSelectionChange(acc.id, "importHistory", !!c)
                        }
                        disabled={state.action === LinkAction.CREATE}
                      />
                      <div className="space-y-1 leading-none">
                        <Label
                          htmlFor={`history-${acc.id}`}
