...
describe("Transaction Synchronization Phase (Advanced)", () => {
  ...
  const setupData = async (
    ...
    await db.user.create({
      data: { id: userId, email: userId + "@test.com", passwordHash: "h" },
    });
  ...
  it("first new account uses longest, first existing uses transactionImportFrom", async () => {
    ...
    const getTxsMock = vi
      .spyOn(EnableBankingClient.prototype, "getTransactions")
      .mockResolvedValue({ transactions: [], skippedInvalid: 0 });

    await syncTransactions({ accountId: "acc-1" });
    expect(getTxsMock).toHaveBeenLastCalledWith(
      "uid-1",
      expect.objectContaining({ strategy: "longest", dateFrom: undefined }),
    );
    ...
    expect(getTxsMock).toHaveBeenLastCalledWith(
      "uid-1",
      expect.objectContaining({ strategy: "default", dateFrom: "2024-01-01" }),
    );
  });
  ...
  it("last sync 30 days ago requests from lastSync - 7 days, NOT today - 7", async () => {
    ...
    await db.externalAccountMapping.update({
      where: { id: mapping.id },
      data: { lastTransactionSyncedAt: lastSync },
    });

    await syncTransactions({ accountId: "acc-1" });
    expect(getTxsMock).toHaveBeenLastCalledWith(
      "uid-1",
      expect.objectContaining({ strategy: "default", dateFrom: "2024-05-03" }),
    );
  });
  ...
  it("overlap never crosses transactionImportFrom", async () => {
    ...
    await db.externalAccountMapping.update({
      ...
      data: {
        transactionImportFrom: importFrom,
        lastTransactionSyncedAt: lastSync,
      },
    });
    ...
    expect(getTxsMock).toHaveBeenLastCalledWith(
      "uid-1",
      expect.objectContaining({ strategy: "default", dateFrom: "2024-05-05" }),
    );
  });
  ...
  it("duplicate transaction on second sync is skipped, same entryReference on different mapping allowed", async () => {
    ...
    const mapping2 = await db.externalAccountMapping.create({
      data: {
        bankConnectionId: mapping.bankConnectionId,
        accountId: acc2.id,
        providerAccountUid: "uid-2",
        identificationHash: "h-2",
      },
    });
  ...
  it("failed second pagination page does NOT update lastTransactionSyncedAt", async () => {
    ...
    const updatedMap = await db.externalAccountMapping.findUnique({
      where: { id: mapping.id },
    });
