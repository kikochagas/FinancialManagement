...
export function inferValueShape(value: any): ValueShape {
  ...
  if (/^[A-Z]{2}\d{2}[A-Z0-9]{10,30}$/.test(str.replace(/\s+/g, "")))
    return "IBAN";
  ...
  if (amountMatch) {
    ...
    if (hasEur)
      return isNegative ? "NEGATIVE_EUR_AMOUNT" : "POSITIVE_EUR_AMOUNT";
    return isNegative ? "NEGATIVE_NUMBER" : "POSITIVE_NUMBER";
  }

  const amountMatch2 = str
    .replace(/\s+/g, "")
    .match(/^([+-]?)(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?(€)?$/);
  if (amountMatch2) {
...
export function sampleColumnShapes(
  rows: any[][],
  columnIndex: number,
  maxSamples = 10,
): ValueShape[] {
