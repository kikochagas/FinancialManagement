...
export function getProvenanceLabel(
  ...
  if (source === "deterministic")
    return `Automatic · ${(confidence * 100).toFixed(0)}%`;
  return "Not mapped";
}
...
export function validateMappings(mapping: Record<number, ColumnMapping>) {
  ...
  const hasDate =
    assignedSemantics.includes("DATE") ||
    assignedSemantics.includes("DATETIME");
  ...
  for (const sem of assignedSemantics) {
    if (sem)
      semanticCounts[sem as string] = (semanticCounts[sem as string] || 0) + 1;
  }
...
export function getFriendlyActivityLabel(
