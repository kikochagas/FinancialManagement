...
describe("Protected External Transactions and Accounts", () => {
  ...
  const setupData = async () => {
    await db.user.create({
      data: { id: "u-1", email: "u-1@test.com", passwordHash: "h" },
    });
    await db.category.create({
      data: { id: "cat-1", name: "Cat 1", userId: "u-1", color: "#000000" },
    });
    const normalAcc = await db.account.create({
      data: {
        id: "a-normal",
        userId: "u-1",
        name: "Normal",
    ...
    const linkedAcc = await db.account.create({
      data: {
        id: "a-linked",
        userId: "u-1",
        name: "Linked",
    ...
    const map = await db.externalAccountMapping.create({
      data: {
        id: "m-1",
        bankConnectionId: conn.id,
        accountId: linkedAcc.id,
        providerAccountUid: "u",
        identificationHash: "h",
      },
    });

    const linkedTx = await db.transaction.create({
      data: {
        id: "tx-linked",
        userId: "u-1",
        accountId: linkedAcc.id,
    ...
    await db.externalTransactionMapping.create({
      data: {
        externalAccountMappingId: map.id,
        transactionId: linkedTx.id,
        dedupKey: "entry:1",
      },
    });

    const normalTx = await db.transaction.create({
      data: {
        id: "tx-normal",
        userId: "u-1",
  ...
  it("rejects deletion of linked tx", async () => {
    ...
    expect(res?.serverError).toBe(
      "Bank-synced transactions cannot be deleted.",
    );
  });
  ...
  it("rejects bulk deletion if any is protected", async () => {
    ...
    const stillExists = await db.transaction.findUnique({
      where: { id: "tx-normal" },
    });
  ...
  it("rejects using another user's account for create", async () => {
    ...
    await db.user.create({
      data: { id: "u-2", email: "u-2@test.com", passwordHash: "h" },
    });
  ...
  it("rejects using another user's category", async () => {
    ...
    await db.category.create({
      data: { id: "cat-2", name: "Cat 2", userId: "u-2", color: "#000000" },
    });

    let res2 = await updateTransaction({
      id: "tx-normal",
      categoryId: "cat-2",
    });
    ...
    let res3 = await updateTransaction({
      id: "tx-linked",
      categoryId: "cat-2",
    });
  ...
  it("legacy transfer with linked source + manual dest -> only manual side adjusts", async () => {
    ...
    const accLinked = await db.account.findUnique({
      where: { id: "a-linked" },
    });
    const accNormal = await db.account.findUnique({
      where: { id: "a-normal" },
    });
