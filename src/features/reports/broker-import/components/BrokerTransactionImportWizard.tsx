...
import {
  getProvenanceLabel,
  validateMappings,
  getFriendlyActivityLabel,
} from "../ux-helpers";
import { ColumnMapping, ParsedBrokerTransaction } from "../types";
import {
  importBrokerTransactionsAction,
  mapBrokerColumnsWithAIAction,
  previewBrokerDuplicatesAction,
} from "../actions";
...
export function BrokerTransactionImportWizard({
  accounts,
}: {
  accounts: any[];
}) {
  ...
  const [eventTypeOverrides, setEventTypeOverrides] = useState<
    Record<string, string>
  >({});
  const [distinctEventTypes, setDistinctEventTypes] = useState<
  ...
  const [currentAccountBalance, setCurrentAccountBalance] = useState<
    number | null
  >(null);
  const [existingLedgerBalance, setExistingLedgerBalance] = useState<
    number | null
  >(null);
  const [existingLedgerBalanceSafe, setExistingLedgerBalanceSafe] =
    useState(false);
  ...
  const proceedToMapping = async () => {
    ...
    try {
      const { orchestrateBrokerColumnMapping } =
      ...
      const result = await orchestrateBrokerColumnMapping(
        headers,
        normalizedHeaders,
        fileData,
        headerRowIdx,
      ...
      setStatus(
        result.aiSucceeded
          ? "AI-assisted mapping was used for ambiguous columns."
          : "Columns mapped automatically using known patterns.",
      );
    } catch (err) {
      console.error(err);
  ...
  const parseWithMapping = async () => {
    ...
    try {
      const res = await (
        await import("../parser")
      ).parseBrokerTransactions(fileData, mapping, undefined, headerRowIdx!);

      // Extract distinct event types
      const distinct = Array.from(
        new Set(res.transactions.map((t) => t.rawEventType).filter(Boolean)),
      ) as string[];
      const defaultOverrides: Record<string, string> = {};
      const distinctTypes = distinct.map((raw) => {
        const canonical =
          res.transactions.find((t) => t.rawEventType === raw)?.eventType ||
          null;
  ...
  const confirmEventTypes = async () => {
    ...
    const hasUnmapped = distinctEventTypes.some(
      (dt) => !eventTypeOverrides[dt.raw],
    );
    if (hasUnmapped) {
      setStatus(
        "Please select a canonical event type (or IGNORE) for all raw event types.",
      );
      return;
    }
    ...
    try {
      ...
      setTransactions(
        res.transactions.map((t, idx) => ({
          ...t,
          import: t.valid && t.eventType !== "IGNORE",
          candidateIndex: idx,
        })),
      );
      setStep(5);
  ...
  const proceedToReview = async () => {
    ...
    try {
      const candidates = transactions
        .filter((t) => t.import && t.eventType !== "IGNORE")
        .map((t) => ({
      ...
      const res = await previewBrokerDuplicatesAction({
      ...
      if (res?.data?.success) {
        ...
        if (res.data.currentAccountBalance !== undefined) {
          ...
          if (
            res.data.currentAccountBalance === 0 &&
            res.data.existingLedgerBalanceSafe
          ) {
  ...
  const handleImport = () => {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (res?.data?.success) {
          const clientSkipped = transactions.filter(
            (t) => t.isProbableDuplicate && !t.import,
          ).length;
          setSummary({
  ...
  const hasUnmappedEvents = distinctEventTypes.some(
    (dt) => !eventTypeOverrides[dt.raw],
  );
  ...
  const computedBalance =
    existingLedgerBalance !== null
      ? Math.round(
          (existingLedgerBalance + candidateCalculation.balance) * 100,
        ) / 100
      : null;
  const isBalanceSafe =
    existingLedgerBalanceSafe && candidateCalculation.isSafe;
  ...
  const readyCount = transactions.filter(
    (t) =>
      t.import && !t.isProbableDuplicate && t.valid && t.eventType !== "IGNORE",
  ).length;
  const duplicateCount = transactions.filter(
    (t) => t.isProbableDuplicate,
  ).length;
  const ignoredCount = transactions.filter(
    (t) => t.eventType === "IGNORE",
  ).length;
  const invalidCount = transactions.filter(
    (t) => !t.valid && t.eventType !== "IGNORE",
  ).length;
  ...
  return (
    <Card className="w-full mt-4">
      ...
      <CardContent className="space-y-4">
        {status && (
          <div className="p-3 bg-muted text-sm rounded-md mb-4">{status}</div>
        )}
            ...
            {headerRowIdx !== null ? (
              <>
                <p className="text-sm text-muted-foreground">
                  We detected the row containing your file's column names.
                  Confirm the detected headers below. You normally only need to
                  change this if the preview looks incorrect.
                </p>
                <div className="text-sm border p-3 rounded max-h-32 overflow-auto bg-muted/20">
                  <strong>Detected column headers:</strong>
                  <br />
                  {fileData[headerRowIdx]
                    .map((h) => String(h || "").trim())
                    .join(", ")}
                </div>
                <div className="pt-2">
                  <label className="text-sm text-muted-foreground mr-2">
                    Change detected header row (1-indexed):
                  </label>
                  <Input
                    type="number"
                    min={1}
              ...
              <div>
                <p className="text-sm text-red-500 mb-2">
                  Could not confidently identify the header row.
                </p>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              FinancialManagement needs to understand what each column in your
              broker file represents. We've mapped the columns automatically.
              Review the suggestions below and only change something if it looks
              incorrect.
            </p>
            ...
            {aiAttempted && aiWarnings.length > 0 && (
              <div className="p-3 bg-yellow-50 text-yellow-800 border border-yellow-200 rounded-md text-sm">
                <p className="font-medium mb-1">
                  AI-assisted mapping was unavailable.
                </p>
            ...
            <div className="space-y-2 mt-4">
              {Object.values(mapping).map((m) => {
                ...
                const isDuplicate =
                  m.semantic &&
                  m.semantic !== "IGNORE" &&
                  isValidating.duplicates.includes(m.semantic);
                return (
                  <div
                    key={m.columnIndex}
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleMappingChange(m.columnIndex, e.target.value)
                      }
                    >
                      ...
                      <option value="ORIGINAL_CURRENCY">
                        Original currency
                      </option>
            ...
            {(() => {
              const { isValid, missingRequired, duplicates } =
              ...
              return (
                <div className="text-sm text-red-600 dark:text-red-400 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 rounded-md">
                  {missingRequired && (
                    <p>
                      Please map both a Date/Datetime and a Broker activity
                      type.
                    </p>
                  )}
                  {duplicates.length > 0 && (
                    <p>
                      Duplicate assignments detected. Each mapped property must
                      be unique.
                    </p>
                  )}
                </div>
            ...
            <div className="text-xs text-muted-foreground space-y-1 mt-4 p-3 bg-muted/10 rounded-md">
              <p>
                <strong>Instrument identifier:</strong> Identifies the
                investment, for example an ISIN such as US8740541094 or a ticker
                such as BTC.
              </p>
              <p>
                <strong>Unique transaction ID:</strong> Used to recognize the
                same broker transaction and prevent duplicate imports.
              </p>
              <p>
                <strong>Original broker category:</strong> Preserved from the
                broker for reference. It is not a FinancialManagement
                income/expense category.
              </p>
              <p>
                <strong>Cash amount:</strong> The signed cash value reported by
                the broker. Negative and positive signs are preserved.
              </p>
            </div>

            <Button
              onClick={parseWithMapping}
              disabled={!validateMappings(mapping).isValid}
            >
              Confirm Mappings
            </Button>
          </div>
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Brokers use different names for investment activity.
              FinancialManagement translates those names into standard event
              types so your investment history can be understood consistently
            ...
            {distinctEventTypes.map((dt) => (
              ...
              >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEventTypeOverrides((prev) => ({
                      ...prev,
                      [dt.raw]: e.target.value,
                    }))
                  }
                >
                  ...
                  <option value="ASSET_TRANSFER_OUT">
            ...
            <div className="text-xs text-muted-foreground space-y-1 mt-4 p-3 bg-muted/10 rounded-md">
              <p>
                <strong>Other:</strong> Preserve this activity in the investment
                history without assigning special behavior.
              </p>
              <p>
                <strong>Ignore this activity:</strong> Do not import
                transactions with this source activity type.
              </p>
            </div>

            {hasUnmappedEvents && (
              <p className="text-sm text-red-500">
                Please resolve highlighted event mappings to continue.
              </p>
            )}
            <Button onClick={confirmEventTypes} disabled={hasUnmappedEvents}>
              Confirm Events
            </Button>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Choose the FinancialManagement account that represents this
              brokerage account. Imported investment activity will be associated
              with this account.
            </p>
            ...
            >
              ...
              {localAccounts.map((a) => {
                const isActiveLink =
                  a.isBankConnected ||
                  (a.externalMappings &&
                    a.externalMappings.some((m: any) => !m.disconnectedAt));
                return (
                  <option key={a.id} value={a.id} disabled={isActiveLink}>
                    {a.name} ({a.currency}){" "}
                    {isActiveLink
                      ? "(Accounts connected through Open Banking cannot be used as broker import targets)"
                      : ""}
                  </option>
                );
              })}
              ...
              <div className="p-4 border rounded bg-muted/20 space-y-3 mt-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      className="w-full p-2 border rounded-md h-9 text-sm bg-background text-foreground border-input"
                      value={newAccType}
                    ...
                    onClick={() => {
                      startTransition(async () => {
                        const res = await createAccount({
                          ...
                          currency: newAccCurrency.toUpperCase(),
                          balance: 0,
                        });
                        if (res?.data?.account) {
            ...
            <Button onClick={proceedToReview} disabled={!selectedAccountId}>
              Review Transactions
            </Button>
          </div>
        )}
        ...
        {step === 6 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Review the investment activity that will be added to your
              historical investment ledger.
              <br />
              This import does not change current portfolio quantities or market
              values yet.
            </p>

            <div className="flex gap-4 text-sm bg-muted/20 p-3 rounded-md border">
              <div>
                <strong>Source rows:</strong> {transactions.length}
              </div>
              <div className="text-emerald-600">
                <strong>Ready to import:</strong> {readyCount}
              </div>
              <div className="text-amber-600">
                <strong>Duplicates:</strong> {duplicateCount}
              </div>
              <div className="text-muted-foreground">
                <strong>Ignored:</strong> {ignoredCount}
              </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {transactions.map((tx, idx) => {
                    ...
                    const friendlyActivity = getFriendlyActivityLabel(
                      tx.eventType,
                      tx.rawEventType,
                    );
                    ...
                    return (
                      <tr
                        key={tx.candidateIndex ?? idx}
                        className={`border-b ${isInvalid ? "bg-red-50 dark:bg-red-900/20" : ""} ${tx.isProbableDuplicate ? "bg-amber-50 dark:bg-amber-900/20" : ""} ${tx.eventType === "IGNORE" ? "opacity-50" : ""}`}
                      >
                        <td className="p-2">
                          <input
                            type="checkbox"
                            checked={tx.import}
                            disabled={!tx.valid || tx.eventType === "IGNORE"}
                            onChange={(e) => {
                        ...
                        <td className="p-2 text-right">
                          {tx.unitPrice !== null ? tx.unitPrice : ""}
                        </td>
                        <td className="p-2 text-right">
                        ...
                        <td className="p-2 text-xs">
                          {statusLabel === "Ready" && (
                            <span className="text-emerald-600">
                              Will be added to your investment history.
                            </span>
                          )}
                          ...
                          {statusLabel === "Ignored" && (
                            <span className="text-muted-foreground">
                              Excluded based on your event mapping.
                            </span>
                          )}
                          {statusLabel === "Invalid" && (
                            <span className="text-red-500">
                              Missing information required to safely import this
                              event.
                            </span>
                          )}
                        </td>
            ...
            {isBalanceSafe && computedBalance !== null ? (
              <div className="flex flex-col space-y-2 mt-4 p-4 border rounded bg-muted/10">
                <div className="text-sm">
                  ...
                  <strong>
                    {currentAccountBalance} {accountCurrency}
                  </strong>
                </div>
                <div className="text-sm">
                  Calculated cash balance:{" "}
                  <strong>
                    {computedBalance} {accountCurrency}
                  </strong>
                </div>
                ...
                <div className="flex items-center space-x-2 mt-2 pt-2 border-t border-border">
                  ...
                  />
                  <label
                    htmlFor="update-balance-checkbox"
                    className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                  >
                    Replace current account cash balance with {computedBalance}{" "}
                    {accountCurrency}
                  </label>
                </div>
              </div>
              ...
              <div className="mt-4 p-4 border rounded bg-amber-50 dark:bg-amber-900/10">
                <div className="text-sm text-amber-700 dark:text-amber-400">
                  FinancialManagement could not reliably calculate a cash
                  balance from this activity. Your account balance will not be
                  changed.
                </div>
              </div>
            )}
            ...
            >
              {readyCount === 0 && updateCashBalance
                ? "Update Cash Balance"
                : "Import"}
            </Button>
          </div>
        ...
        {step === 7 && (
          <div className="text-center">
            <h3 className="font-bold">Importing...</h3>
          </div>
        )}
        ...
        {step === 8 && summary && (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              Your broker activity has been processed. Current portfolio values
              are not updated by this import yet.
            </p>
            <div className="flex flex-col justify-center gap-2 mt-4 mb-6 text-sm">
              {summary.imported > 0 || summary.skipped === 0 ? (
                <div className="text-emerald-600 font-bold">
                  {summary.imported} investment activities imported.
                </div>
              ) : (
              ...
              {summary.balanceUpdated ? (
                <div className="text-blue-600 font-bold mt-2">
                  Account cash balance updated to {summary.resultingBalance}{" "}
                  {accountCurrency}.
                </div>
              ) : (
                <div className="text-muted-foreground mt-2">
                  Account cash balance was left unchanged.
                </div>
              )}
            </div>
            <div className="flex justify-center gap-4">
              <Button
                onClick={() =>
                  (window.location.href = "/investments?tab=activity")
                }
              >
