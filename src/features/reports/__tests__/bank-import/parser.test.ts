...
describe("parser orchestration", () => {
  it("A. AI not needed (Pure deterministic parsing when confidence is high)", async () => {
    ...
    const { mapping, aiAttempted, aiSucceeded, aiError, warnings } =
      await orchestrateColumnMapping(headers, dataRows, mapper);
  ...
  it("B. AI request throws", async () => {
    ...
    const mockProvider: AIProvider = {
      generateStructured: vi
        .fn()
        .mockRejectedValue(new Error("credit_balance_exhausted")),
    };
    const mapper = new BankStatementAIMapper(mockProvider);

    const { aiAttempted, aiSucceeded, aiError } =
      await orchestrateColumnMapping(headers, dataRows, mapper);
  ...
  it("D. Local AI merge fails (Invalid mappings structure)", async () => {
    ...
    const { aiAttempted, aiSucceeded, aiError, warnings } =
      await orchestrateColumnMapping(headers, dataRows, mapper);
    ...
    // ...but merge failed
    expect(aiSucceeded).toBe(false);
    expect(warnings.some((w) => w.includes("Local AI merge failed"))).toBe(
      true,
    );
