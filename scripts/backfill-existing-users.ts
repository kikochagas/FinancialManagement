...
if (
  ...
  console.error(
    "ERROR: This legacy migration script is intended for local/explicit legacy database migration and must not run against the hosted Turso demo database.",
  );
  process.exit(1);
}
...
if (
  ...
  console.error(
    "ERROR: A local SQLite DATABASE_URL starting with 'file:' is required.",
  );
  process.exit(1);
}
...
export async function backfillTransactions(
  ...
  console.log(
    `Migration Complete. Updated: ${updatedCount}, Skipped/Already Migrated: ${skippedCount}`,
  );
}
...
if (
  ...
  (typeof process !== "undefined" &&
    process.argv[1]?.endsWith("backfill-existing-users.ts"))
) {
