...
export function parseMoneyStrict(input: any): ParseResult<number> & {
  ...
  if (typeof input === "number") {
    if (!isFinite(input))
      return { valid: false, value: null, warning: "Amount is not finite" };
    return {
      valid: true,
      value: Number(input.toFixed(2)),
      explicitSign: input < 0 ? "negative" : null,
    };
  }
  ...
  str = str.replace(/\s+/g, "");
  if (str === "")
    return { valid: false, value: null, warning: "Empty amount string" };
  ...
  const isUS = /^[-+]?(?:\d{1,3}(?:,\d{3})*(?:\.\d+)?|\d+(?:\.\d+)?)$/.test(
    str,
  );
  ...
  if (!isEU && !isUS) {
    return {
      valid: false,
      value: null,
      warning: `Invalid amount format: ${input}`,
      explicitSign,
      currency,
    };
  }
