...
export async function getAccountsData() {
  ...
  return {
    accounts: accounts.map((a) => {
      const activeMapping = a.externalMappings.find(
        (m) => m.disconnectedAt === null,
      );
      const anyMapping =
        a.externalMappings.length > 0 ? a.externalMappings[0] : null;
      ...
      return {
        ...
        recentTransactions: [...a.transactions, ...a.transferTransactions]
          ...
          .map((t) => {
            ...
            if (t.direction === "InternalTransfer") {
              if (t.accountId === a.id)
                desc = `Transfer to ${t.destinationAccount?.name || "External"}`;
              else if (t.destinationAccountId === a.id)
                desc = `Transfer from ${t.account?.name || "External"}`;
            }
            return {
              id: t.id,
        ...
        institutionName: activeMapping
          ? activeMapping.bankConnection.institutionName
          : anyMapping
            ? anyMapping.bankConnection.institutionName
            : null,
        connectionStatus: activeMapping
          ? activeMapping.bankConnection.status
          : null,
        validUntil: activeMapping
          ? activeMapping.bankConnection.validUntil?.toISOString() || null
          : null,
        lastBalanceSyncedAt: activeMapping
          ? activeMapping.lastBalanceSyncedAt?.toISOString() || null
          : null,
        lastTransactionSyncedAt: activeMapping
          ? activeMapping.lastTransactionSyncedAt?.toISOString() || null
          : null,
      };
    }),
  };
