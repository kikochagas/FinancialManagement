...
describe("Bank Statement Import Orchestration", () => {
  it("handles Crédito Agrícola description collision correctly", async () => {
    ...
    const { mapping, warnings } = await orchestrateColumnMapping(
      headers,
      rows,
      mockAiMapper,
    );
    ...
    const descCols = Object.values(mapping).filter(
      (m) => m.semantic === "DESCRIPTION",
    );
    expect(descCols.length).toBeGreaterThan(1);
    expect(
      warnings.some((w) => w.includes("Semantic collision on DESCRIPTION")),
    ).toBe(true);
    ...
    Object.values(resolvedMapping).forEach((m) => {
      ...
      else if (m.header === "Saldo após movimento")
        m.semantic = "BALANCE_AFTER";
      else if (m.header === "Tipo") m.semantic = "TYPE";
    });

    const { transactions, endingBalance, footerRowsSkipped } =
      buildTransactions(rows, 0, resolvedMapping as any);
    ...
    expect(validTxs[0].amount).toBe(128.0);
    ...
    const invalidTx = transactions.find(
      (t) => t.description === "INVALID CALENDAR DATE",
    );
    expect(invalidTx).toBeDefined();
    expect(invalidTx?.valid).toBe(false);
    expect(
      invalidTx?.warnings.some((w) => w.includes("Booking Date is required")),
    ).toBe(true);
