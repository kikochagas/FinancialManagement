...
} from "../../broker-import/ux-helpers";

describe("broker-import ux-helpers", () => {
  test("getProvenanceLabel handles sources correctly", () => {
    expect(getProvenanceLabel("deterministic", 0.95, "DATE")).toBe(
      "Automatic · 95%",
    );
    expect(getProvenanceLabel("ai", 0.92, "QUANTITY")).toBe(
      "AI-assisted · 92%",
    );
    expect(getProvenanceLabel("user", 1.0, "FEE")).toBe("Selected by you");
    expect(getProvenanceLabel("deterministic", 0.5, "UNMAPPED")).toBe(
      "Not mapped",
    );
  ...
  test("getFriendlyActivityLabel maps correctly", () => {
    ...
    expect(getFriendlyActivityLabel("ASSET_TRANSFER_IN", null)).toBe(
      "Asset received",
    );
