...
describe("broker-import deduplication", () => {
  test("external_id is respected and preferred", () => {
    const key1 = generateDedupKey("acc1", {
      externalId: "EXT-123",
      occurredAt: "2024-01-01",
    } as any);
    const key2 = generateDedupKey("acc1", {
      externalId: "EXT-123",
      amount: 100,
      occurredAt: "2024-01-01",
    } as any);
  ...
  test("fallback dedup uses instrument identity", () => {
    ...
    expect(generateDedupKey("acc1", tx1 as any)).toBe(
      generateDedupKey("acc1", tx2 as any),
    );
    expect(generateDedupKey("acc1", tx1 as any)).not.toBe(
      generateDedupKey("acc1", tx3 as any),
    );
  });
});
...
describe("broker-import mapping and AI orchestration", () => {
  ...
  test("order_id is NOT treated automatically as external ID", () => {
    const mapping = mapBrokerColumnsDeterministically(
      ["order_id"],
      ["order_id"],
    );
  ...
  test("order_id remains unmapped even if AI somehow suggests EXTERNAL_ID", async () => {
    ...
    const deterministic = mapBrokerColumnsDeterministically(
      headers,
      normalized,
    );
    ...
    expect(result.warnings.some((w) => w.includes("disabled for safety"))).toBe(
      true,
    );
  });
});
