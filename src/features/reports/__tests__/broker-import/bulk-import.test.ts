...
describe("broker-import bulk deduplication and insertion", () => {
  ...
  beforeAll(async () => {
    user = await db.user.create({
      data: {
        email: "test-bulk-dedup@example.com",
        name: "Test Bulk Dedup",
        passwordHash: "dummy",
      },
    });
  });
  ...
  test("73-event style batch import succeeds", async () => {
    const candidates = Array.from({ length: 73 }).map((_, i) =>
      generateMockEvent(i + 1),
    );
    const result = await importBrokerTransactionsForUser(
      user.id,
      account.id,
      candidates,
      false,
    ...
    const dbCount = await db.investmentEvent.count({
  ...
  test("Existing duplicates are excluded in bulk", async () => {
    // insert 70 first
    const first70 = Array.from({ length: 70 }).map((_, i) =>
      generateMockEvent(i + 1),
    );
    await importBrokerTransactionsForUser(user.id, account.id, first70, false);

    // try to insert 73
    const all73 = Array.from({ length: 73 }).map((_, i) =>
      generateMockEvent(i + 1),
    );
    const result = await importBrokerTransactionsForUser(
      user.id,
  ...
  test("transaction failure still rolls back all writes", async () => {
    ...
    await expect(
      importBrokerTransactionsForUser(user.id, account.id, events, true),
